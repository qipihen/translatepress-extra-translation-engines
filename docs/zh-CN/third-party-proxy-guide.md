...
例如你提供的这类接口文档（仅示例、无商业合作）：

- `https://tb.trans-home.com/index/index/api`
- 批量接口：`/api/index/translateBatch?token=...`

本插件对应引擎：`Third-Party Proxy API (Single Token, Example)`（engine value: `transhome_translate`）。

## 1. 后台怎么填

在 `TranslatePress -> Automatic Translation`：

1. `Alternative Engines` 选择 `Third-Party Proxy API (Single Token, Example)`
2. 填 `Proxy API Token`
...
填 `Proxy API Token`，不要填 Youdao/Baidu 的双字段。
